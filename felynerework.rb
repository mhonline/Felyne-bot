...
#Setup maintenance
bot.command(:mainsetup, permission_level: 1, description: "set up maintenance timer", usage: "-maintenance <hours> <minutes>") do |event, hours, minutes|
	h = hours.to_i
	m = minutes.to_i
	time = h * 3600 + m * 60
...
bot.command(:kill, description: "kills felyne", permission_level: 800) do |event|
  puts "Bot restarting"
  bot.send_message(event.message.channel, "Why you kill me you monster!!")
  bot.stop
  exit
end
