module Commands
  ...
  module NewGuild
    ...
    command(
      ...
      permission_message: 'Talk to the server admin if you have a new guild you
      would like added to the server.'
    ) do |event, search, server|
      if event.user.can_manage_roles?
        server_role = event.server.roles.find { |role| role.name == search }
        if server_role.nil?
          event.respond "The role **#{search}** does not exist on the server.
          Please create it before running the command again."
        else
