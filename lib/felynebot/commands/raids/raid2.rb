module FelyneBot
	module Commands
		module Raid2
			...
			command(
					:raid2,
					description: "Schedules Raid 2. (Name has to be a single word)",
					usage: "raid2 <name/clear> <days> <hours> <minutes>",
					permission_level: 1,
				...
				if name == "clear"
					...
					event.respond "Raid 2 has been cleared"
				else
					File.write('bot/raid2', name)
					open('bot/raid2', 'a') { |f|
						f.puts "\n#{t4}"
					}
					event.respond "#{d2} days #{h2} hours #{m2} minutes until #{name} raid"
				end
				puts "#{event.timestamp}: #{event.user.name}: CMD: raid2 <#{name}> <#{t4}>"
			end
		end
	end
end